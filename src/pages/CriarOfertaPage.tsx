...
const CriarOfertaPage: React.FC = () => {
  ...
  // Aguarda pmeData carregar antes de inicializar o form com coordenadas corretas
  if (!pmeData) return null;
  ...
  const {
    ...
    pmeLat: pmeData?.geo?.latitude ?? pmeData?.endereco?.geo?.latitude ?? -23.5505,
    pmeLng: pmeData?.geo?.longitude ?? pmeData?.endereco?.geo?.longitude ?? -46.6333,
